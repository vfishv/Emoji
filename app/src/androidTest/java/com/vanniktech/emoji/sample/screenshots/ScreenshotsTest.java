...
@RunWith(AndroidJUnit4.class) public final class ScreenshotsTest {
  ...
  private static void start(final Variant variant) throws InterruptedException {
    ...
    onView(withId(R.id.chatEditText)).perform(appendText("Hello what's up? " + new String(firstEmojis, 0, firstEmojis.length)));
    ...
    onView(withId(R.id.chatEditText)).perform(appendText(new String(beerEmojis, 0, beerEmojis.length)));
    onView(withId(R.id.chatSend)).perform(click());

    final int[] clinkingBeerEmoji = new int[] { 0x1F37B };
    onView(withId(R.id.chatEditText)).perform(appendText(new String(clinkingBeerEmoji, 0, clinkingBeerEmoji.length)));
    onView(withId(R.id.chatSend)).perform(click());
    ...
    onView(withId(R.id.chatEditText)).perform(appendText("I don't know " + new String(secondEmojis, 0, secondEmojis.length)));
